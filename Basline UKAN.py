...
class KANLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = x.reshape(B, N, self.fc1.out_features).contiguous()
        x = self.dwconv_1(x, H, W)

        x = self.fc2(x.reshape(B * N, self.fc1.out_features))
        x = x.reshape(B, N, self.fc2.out_features).contiguous()
        x = self.dwconv_2(x, H, W)

        x = self.fc3(x.reshape(B * N, self.fc2.out_features))
        x = x.reshape(B, N, self.fc3.out_features).contiguous()
...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3, embed_dims=[256, 320, 512], no_kan=False,
        ...
        self.block2 = nn.ModuleList([KANBlock(
            dim=embed_dims[2],
            drop=drop_rate, drop_path=dpr[sum(depths[:1]) + i], norm_layer=norm_layer
            ) for i in range(depths[1])])

        self.dblock1 = nn.ModuleList([KANBlock(
            dim=embed_dims[1],
            drop=drop_rate, drop_path=dpr[sum(depths[:2]) + i], norm_layer=norm_layer
        ...
        self.soft = nn.Sigmoid()  # Changed to Sigmoid for binary segmentation
    ...
    def regularization_loss(self, regularize_activation=1.0, regularize_entropy=1.0):
        ...
        for module in self.modules():
            if isinstance(module, KANLinear):
                total_loss += module.regularization_loss(regularize_activation, regularize_entropy)
        return total_loss
